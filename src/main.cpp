...
void clear_output_text_file(graphnode * g, string outputfile)
{
    std::ofstream ofs;
...
void writeMedianToOutputFile(graphnode * g,string outputfile)
{
    std::ofstream ofs;
    ...
    ofs.open(outputfile.c_str(), std::ios_base::app); // appends to text file.
    ofs << std::fixed << std::setprecision(2) << medianFinal << std::endl; // so every median is in x.xx format .
    ofs.close();
}

void parse_string(string s, graphnode *g, string outputfile)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        if (i == 5) // parse actor
        {
            temp = temp.substr(0,temp.size()-1); // remove right bracket
            if (temp.size() > 2) // is not ""
            {
                ...
                g->getDegrees();
                g->storeMedian();
                writeMedianToOutputFile(g,outputfile);
            }
        }
    }
    ++counter;
...
int main(int argc, char **argv)
{
   ...
   // std::cout << inputfile << " " << outputfile << std::endl;
    //inputfile = "venmo_input/venmo-trans.txt";
    //outputfile = "venmo_output/output.txt";
    
    clear_output_text_file(g,outputfile);



    std::ifstream transactions(inputfile.c_str());
    if (transactions.is_open())
    {
        while (getline(transactions,line))
        {
            parse_string(line, g,outputfile);
        }
        transactions.close();
    }
    else
